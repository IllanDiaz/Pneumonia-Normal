...
def download_and_extract_model():
    model_url ='https://dl.dropboxusercontent.com/s/dq3uf4bcjjxpk6z2aeuhp/mobilenet_v2_model.zip?rlkey=fk38ttvmutvohu7x9b6vnarlo&st=vxec38di'
    ...
    return os.path.join(extract_folder, 'mobilenet_v2_model (1).keras')
...
if uploaded_file is not None and model is not None:
    ...
    if prediction[0][0] > 0.5:
        st.success('El modelo predice que la imagen es un menor con **pulmonia**.')
    else:
        st.success('El modelo predice que la imagen es un menor **sano**.')